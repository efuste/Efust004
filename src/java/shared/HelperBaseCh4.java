...
public abstract class HelperBaseCh4 {
    ...
    protected void initLogger() {
        ...
        logger.info("Called HelperBaseCh4 initLogger()");

    }

    protected abstract void copyFromSession(Object helper);

    protected void addHelperToSession(String name, SessionData state) {
    ...
    protected void fillBeanFromRequest(Object data) {
        try{
            BeanUtils.populate(data, request.getParameterMap());
        } catch (IllegalAccessException ex) {
            logger.error("Populate illegal access", ex);
        } catch (InvocationTargetException ex) {
            logger.error("Populate invocation target", ex.getTargetException());
        }
    }
}
